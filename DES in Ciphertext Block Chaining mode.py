...
START_VECTOR = 'AAB4C7EF9DB3D421' # 64 битный начальный вектор 
...
def Encrypt(src):
    ...
    for i in WrapSrc(src):
        ...
        if len(encrypted_list) == 0: bin_mess = XOR(bin_mess, to_bin(START_VECTOR)) #Если шифруется первый блок, берем для этого блока XOR по вектору инициализации
        else: bin_mess = XOR(bin_mess, LastBlock) #Если шифруется не первый блок, берем XOR по прошлому зашифрованному блоку
        ...
        LastBlock = ''.join(des(permuted_block, key_list)) #Сохраняем текущий защифрованный блок чтобы использовать его в следующей итерации в XOR
        encrypted_list.append(''.join([hex(int(i, 2))[2:].zfill(2).upper() for i in wrap(LastBlock, 8)]))
...
def Decrypt(src):
    ...
    for i in wrap(src, 16):
        ...
        TempBlock = ''.join(des(permuted_block, reversed(key_list))) #Результат дешифрации до метода XOR

        if len(temp_li) == 0: temp_li.append(''.join([hex(int(i, 2))[2:].zfill(2).upper() for i in wrap(XOR(TempBlock, to_bin(START_VECTOR)), 8)])) #Если дешифруется первый блок,
        #берем для этого блока XOR по вектору инициализации
        else: temp_li.append(''.join([hex(int(i, 2))[2:].zfill(2).upper() for i in wrap(XOR(TempBlock, LastBlock), 8)])) #Если дешифруется не первый блок,
        #берем XOR по прошлому зашифрованному блоку

        LastBlock = bin_mess #Сохраняем текущий защифрованный блок чтобы использовать его в следующей итерации в XOR
